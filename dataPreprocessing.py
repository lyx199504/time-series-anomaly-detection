...
def getNabDataset(dataset_name, seq_len=50, move_pace=1, pre_list=(standard,), deal_list=()):
    ...
    for i, path in enumerate(path_list):
        ...
        p_ = np.array([np.where(raw_label[ix: ix + seq_len] == 1)[0][0] + 1 if sum(raw_label[ix: ix + seq_len]) > 0 else 0 for ix in range(s_point, e_point, move_pace)])
        p.append(p_)

        # y_ = np.array([1 if sum(raw_label[ix: ix + seq_len]) > 0 else 0 for ix in range(s_point, e_point, move_pace)])
        y_ = np.where(p_ > 0, 1, 0)
        y.append(y_)

    X, y, p = np.vstack(X).astype('float32'), np.hstack(y).astype('int32'), np.hstack(p).astype('int32')
...
# 获取AIOps数据集，将异常数据定为正例
def getAIOpsDataset(dataset_name, seq_len=50, move_pace=1, pre_list=(standard,), deal_list=()):
    X, y, p = [], [], []
    path_list = list(Path("./datasets/AIOps/%s" % dataset_name).glob("*.csv"))
    for i, path in enumerate(path_list):
        raw_data = pd.read_csv(path)
        try:
            raw_value = raw_data.value.values
...
def getDataset(dataset_name, seq_len=50, move_pace=1, pre_list=(standard,), deal_list=()):
    ...
    elif dataset_name.startswith('K'):
        return getAIOpsDataset(dataset_name, seq_len, move_pace, pre_list, deal_list)
    else:
